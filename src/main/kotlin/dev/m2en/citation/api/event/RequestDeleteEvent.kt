...
class RequestDeleteEvent : ListenerAdapter() {
    ...
    private fun sendResult(interaction: ButtonInteraction, message: String?, error: Boolean = false) {
        if(error) {
            interaction.replyEmbeds(EmbedBuilder.buildErrorEmbed("削除に失敗しました。", message)).setEphemeral(true).queue()
        } else {
            interaction.replyEmbeds(EmbedBuilder.buildEmbed("削除に成功しました。", message)).setEphemeral(true).queue()
        }
    }
}
